...
- **Stage:** 3 `/tdd` grind — **ALL 9 SLICES DONE (S1–S7, no S6c split needed).** → **next: stage 4 `/code-review`.** Whole tree green: `./dev test` EXIT:0; all execute-integration tests (test_per_group_timeout, test_max_jobs_overlap, test_composition_s7) pass directly. **UNCOMMITTED** — all RFC-0002 work is in the working tree; commit when Corey asks.
- **RFC:** `docs/rfc/0002-scheduling-and-config-correctness.md` (699 lines; round-1 + round-2 changelogs at end).
- **Resume (stage 4):** `/code-review docs/rfc/0002-scheduling-and-config-correctness.md`
