...
- [ ] **D2** `lock` (`LockFileEx`), `ioutils` (`MoveFileEx`, `CREATE_NEW`), `signals` (`SetConsoleCtrlHandler` → `ShutdownSignal`), memprobe (Job accounting via `groupRssBytes` + `GlobalMemoryStatusEx`). `windows-latest` full conformance leg. **This slice owns test/fixture portability** (35 test files under `tests/` import `std/posix` today, incl. 2 fixtures — measured; RFC-0009 owns only path identity, not de-POSIXing the suite) — the A3 grep-assert extends to `tests/` for Windows-relevant usage; full-suite green remains RFC-0009's acceptance. When Stage D is re-cut for execution, split D2a (production: lock/ioutils/signals/memprobe) from D2b (the test-suite sweep). **Scoping notes (2026-09-10 D2-surface map):** (a) **`httpraw.nim` is an additional, unnamed `--os:windows` production blocker** — it `import std/posix` for `posix.Time` (SO_RCVTIMEO/SO_SNDTIMEO timeval, reachable via `cachehttp.nim`'s remote-fetch path); D2a will include it (a small `when defined(windows)` branch) unless scoped out. `signals.nim` needs NO change — the real gap is `process/windows.nim` missing a `globalShutdownSignal()` getter (the CTRL handler + Event already exist); the D2 `groupRssBytes` sampler is `process/windows.nim`'s stub (sum `snapshotOnePid.rssBytes` over `queryJobPids` — plumbing exists), while `availableMemBytes` needs a `GlobalMemoryStatusEx` branch in `memprobe.nim`; `ioutils.nim`'s `cint`-fd type threads through many call sites (the hard sub-slice — needs an opaque handle type). (b) **The "35 test files" D2b count is stale — a fresh grep finds 73** across conformance/fixtures/integration/timing/unit; D2b must re-measure. (c) D2b full-suite green stays RFC-0009-gated (the escalation boundary after D2a).
