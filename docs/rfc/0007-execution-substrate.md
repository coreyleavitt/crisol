...
- [ ] **D2b** Test-suite portability sweep + `windows-latest` FULL conformance leg + full-suite windows green. **RFC-0009-GATED (the escalation boundary — NOT deferred, blocked by design):** the remaining `--os:windows` work is de-POSIXing the test suite itself — a fresh grep finds **~73** `tests/**` files importing `std/posix` (the bullet's original "35" was stale/measured earlier), and path-identity (which RFC-0009 owns) must land before the suite can be de-POSIX'd and run green on windows. Per this RFC's own acceptance wording, "full-suite green remains RFC-0009's acceptance." D2a proved the PRODUCT runs on windows via a self-contained subprocess CLI smoke that needs none of the std/posix-importing test helpers; D2b is where the windows leg is widened to the whole suite, under RFC-0009.
