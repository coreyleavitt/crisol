...
**Status:** Build complete — Stages A–C done; Stage D production complete (D1a/D1b kill-domain + forensics; D2a Windows production de-POSIXing — `crisol run` compiles+runs on windows, CI-green all five legs, 2026-09-10). **D2b** (test-suite de-POSIX + windows conformance-leg green) DONE under RFC-0009 (CI 35170572732, 2026-09-16) — Stage D complete. **D1c** (cbLimit attribution on windows, the D1b follow-up) signed off, built, and CI-proven 2026-09-18 (05a90cb; all 5 legs green, run 35353539907 — the windows leg carries the behavioral proof).
...
type
  ...
  ChildSpec* = object
    argv*: seq[string]
    claimOrphans*: bool = true  ## containment intent, declared once at spawn
                             ## (review r28, 2026-09-19): at reap, reparented
                             ## orphans / same-pgroup survivors discovered on
                             ## this child's account may be claimed and killed
                             ## as its escapees. The runner sets false for
                             ## compile children (toolchain transients are
                             ## contract-exempt). Replaces the undocumented
                             ## B1-era reap(id, runPhase) deviation; reap's
                             ## pinned signature reap(sv, id) is restored.
...
type
  ReapReport* = object       ## the ONE report: the backend's complete observation
    ...
    memoryPeakBytes*: Option[int64]  ## (w4, 2026-09-19): the cgroup tier's own
                             ## `memory.peak` leaf-wide peak RSS readback —
                             ## forensics only, the ledger's tagged-successor
                             ## `maxRssBytes`/`rssMechanism` producer (§7);
                             ## `none` off the cgroup tier. Admission is
                             ## unaffected — memprobe's sampled group-sum
                             ## stays the admission quantity.
```
...
|---|---|---|---|---|
| Linux | pgid; crisol sets `PR_SET_CHILD_SUBREAPER` so orphans at any depth reparent to crisol and are reaped + counted; per-slot cgroup v2 leaf via child self-join to `cgroup.procs` when delegated (`clone3(CLONE_INTO_CGROUP)` rejected — unusable from Nim's fork/exec window, see B3; `cgroup.kill` is atomic and airtight; `memory.max` backs the separate `lkMemory` kind — `RLIMIT_AS` stays, §2 note; `memory.peak` is probed/reported only) | `cgroup` / `processGroup+subreaper` | SIGTERM → SIGKILL | pidfd + poll/epoll (≥ 5.3), 25 ms poll fallback |
...
rlimits remain the floor, mapped through `Limits` — the single home; `SandboxSpec` sheds its rlimit fields, its env/tmpdir/cwd content is resolved by the runner into `ChildSpec.env`/`cwd` before spawn (achieved-by-construction; the child-side status pipe shrinks to per-limit readback), and **`SandboxAchieved` is deleted outright** — every field is obsolete (`rlimitsApplied` → `LimitsAchieved`; env/tmpdir true by construction; `netIso` never wired and never-goal), and `Evidence.hermetic` carries the level (§2). The re-homing changes the SoundnessKey's fold-input shape (`keys.nim` serializes the config-declared rlimits): deliberate, and free — the `resultCacheFormatVersion` bump discards all entries anyway (A2a-iii names `keys.nim`). Linux adds cgroup `memory.max` as the separate `lkMemory` kind when delegated (`RLIMIT_AS` stays, deliberately generous for ORC — §2 note; the mechanism used is reported). Windows maps `Limits` to Job basic/extended limits; `openFiles` has no analog and is reported `lsUnsupported`. **No network enforcement anywhere** — see §Non-goals. **No mount/user namespaces** — deferred; `TMPDIR` injection plus RFC-0008's observed inputs cover the soundness case without a second isolation mechanism.
...
- **Rusage is a new quantity, not a replacement.** `wait4`'s `ru_maxrss` is a per-process max (folded over reaped descendants), while `memprobe.procGroupRssBytes` — the value admission estimates and the ledger's `rssBytes` consume today — is a *simultaneous tree sum*. They are different quantities, and for exactly the workloads admission exists for (`nim c` + cc children) the per-process max undercounts. So: the sampled group-sum **remains** the ledger/admission quantity; `wait4` maxRss lands in `ProcessResult.rusage` and a new, mechanism-tagged ledger column; the cgroup tier's `memory.peak` (a true tree peak, B3) supersedes wait4 in that SAME ledger column, explicitly and tagged, never silently — landed w4 (2026-09-19): `ReapReport.memoryPeakBytes` (reap-time read, `process/cgroup.nim`'s `cgroupLeafMemoryPeak`) feeds `ledgerRssObservation` (`ledger.nim`), the one site the `maxRssBytes`/`rssMechanism` preference is decided — `"memory.peak"` wins over `"wait4"` whenever the cgroup tier produced a reading, tagged accordingly; the sampled group-sum itself is still never touched. Sampling itself routes through the contract — `groupRssBytes(sv, id)` (§1), whose posix implementation *is* today's memprobe pgid scan re-homed — because the executor no longer holds a `Pid` to hand memprobe (A2b owns the rewiring; RFC-0002's quantity and cadence are unchanged — §Contract impacts).
...
- [x] **B1** `PR_SET_CHILD_SUBREAPER` with the §3 mechanics: SIGCHLD + `waitid(P_ALL, WNOWAIT)` sweep beside the wait set, pgid attribution via `/proc/<pid>/stat` before reap, `weOrphanReaped` events (with `ownedBy` delivered — §1); live escapees killed via `pidfd_open` + starttime identity check; `killDomain = processGroupSubreaper`. Fixture `spawn_grandchild_setsid` now shows `tree = toComplete` with `escapees` non-empty (observed, killed, reaped, counted — and per §6 still uncacheable: the axes are separate). ✅ 5728246 + 7194e21; CI 34327304618 green all four legs (2026-09-09). Bounded WNOHANG reap (no event-loop starvation); reparented-orphan discovery scoped to run-phase reaps; waitid-sweep-vs-registered-reap livelock fixed; run/v2 rev 24 (`lateOrphansReaped`).
- [x] **B2** pidfd + epoll-driven `next` with timerfd deadlines; 25 ms tick retained only for memory sampling; falls back to polling when pidfd is absent (probe). Conformance suite green under both (forced via an env knob in tests). ✅ 0bb5f99; CI 34339945448 green all four legs (2026-09-09). epoll+timerfd loop with poll fallback; CRISOL_FORCE_POLL knob runs conformance under both modes in CI; epollBlock never inspects returned events so pollSweepChildren stays the single reap path (B1 race guarantee preserved); a2b 10/10 both modes, fd-leak + latency tests added.
- [x] **B3** cgroup v2 per-slot leaf when delegated. **First checklist item: the CI leg that actually has delegation** (e.g. a `systemd-run --user --scope`-wrapped or privileged-container job), proven by a canary test that *fails* if the probe reports delegation absent — the backend code does not merge before an environment that executes it exists (the #12 lesson, applied in advance; rootless podman runs only the degradation branch). Then: `clone3(CLONE_PIDFD|CLONE_INTO_CGROUP)`, `cgroup.kill` for forced kill, `memory.peak` probed/reported only — the §7 tagged-successor tree accounting landed separately as item w4 (2026-09-19; see §7), `memory.max` for the separate `Limits.req[lkMemory]` kind (not `lkAddressSpace` — §2 note; mechanism reported; the ENOMEM→SIGKILL failure-mode change documented); **adds `lkMemory`, asserted only when the child's own `Exit` is `ekSignaled(SIGKILL)` ∧ `oom_kill > 0`** — a grandchild's OOM kill that the test tolerated (child exited on its own) is recorded as an `Evidence`-level count, never as `Cause`: authorship of an ending that did not happen would contradict `Exit`; `killDomain = cgroup`. Tests self-gate on delegation and assert *degradation* where absent; a green-probe/failed-leaf **fault-injection conformance test** (delegation probed green, leaf mkdir forced to fail — e.g. read-only leaf parent) asserts the spawn proceeds on the fallback tier and the *result* carries the degraded `killDomain` — the per-spawn-achieved motivation (§4), exercised, not just stated. ✅ leg-first: phase-1 (cgroup CI leg + fail-on-absent canary) 05815ca/a60898a (CI 34374079270), phase-2 backend 5254ea4 + fixes e8a30d0/cd24727/62fe2a2/c208b62; CI run 34404878358 green ALL FIVE legs incl. the `cgroup` leg (2026-09-09). Per-slot cgroup-v2 leaf via child self-join to `cgroup.procs` (clone3(CLONE_INTO_CGROUP) documented-as-rejected from Nim's fork/exec window); `cgroup.kill` teardown; leaf `cgroup.procs` survivors captured into `escapees` then killed+reaped (subreaper reparenting) before rmdir; `memory.events` oom_kill drives `lkMemory` ONLY when the child's OWN Exit=SIGKILL; `killDomain=kdsCgroup` (tree=toComplete) with PER-SPAWN honest degrade to kdsProcessGroupSubreaper when a leaf's self-join fails after a green probe. Fault-injection sabotage went through three empirically-disproven forms (regular file — cgroupfs rejects O_CREAT; pids.max=0 — gates forks not migrations; +cpu-only — checked at enable-time) before the working one: a leaf turned INTERNAL (child cgroup + `+memory` subtree_control) which the no-internal-process rule forbids from holding processes, verified end-to-end against a --privileged cgroup-v2 container.

**Stage C — Darwin backend (leg exists since A2d):**
- [x] **C1a** `process/posix` green on the `macos-latest` leg via the poll fallback: full conformance suite + full unit suite (near-zero new code — this proves the CI loop before behavior work starts). ✅ commits 6df6d26 (macos job wired to `CRISOL_TEST_DIRS=tests/unit:tests/conformance bash ci/run-tests.sh`, tier `macos`) · fa958d6/f5cb84f (genuine cross-platform fixes: `runner.nim` `mkdtemp(3)` FFI → `std/tempfiles.createTempDir`; `closure.nim` records each root's realpath form for macOS `/var→/private/var`; `test_ccprobe` findExe/realpath) · 24257c1 (`test_jsonout` per-backend substrate key set). macOS tier-guards: `test_process_capabilities` (macos arm: subreaper/pidfd/cgroupDelegation false, flock/wait4Rusage true; prctl cross-check `when defined(linux)`), `test_rfc0007_a6a_escapee_evidence` (skip when ¬subreaper∧¬pidfd), `test_compiledriver` (macOS-widened ccSpanUs). CI run 34423297420 green ALL FIVE legs; macos ran the full suite `OK: 115 file(s)`. No `src/crisol/process/*` behavior added (that is C1b).
- [x] **C1b** `process/darwin.nim` born as a THIN DELEGATION module over `posixcore` (§1 sharing mechanism — Nim has no partial module override): its own `Supervisor` embedding `PosixCore`; kqueue `EVFILT_PROC` `next`; libproc `snapshotTree`/`groupRssBytes` with rssBytes — admission sampling routes through the contract, so Darwin is *not* resigned to degradation; the `process.nim` ladder flips `macosx` from posix to darwin here; `capabilities()` gains the darwin probes and `substrate` renders them; `hlNetwork` unachieved-and-reported; setsid escapees honestly `toUnobservable` (§3). tonalli macOS legs convert. ✅ commits dee7370 (darwin.nim pure shell mirroring linux.nim — all macOS mechanism in `posixcore.nim` `when defined(macosx):` branches, the RFC's "own Supervisor embedding PosixCore" satisfied transitively via re-export; kqueue `EVFILT_PROC`/`NOTE_EXIT` event-driven `next` mirroring epollBlock; libproc `walkProcTable`/`readVmRssBytes` via proc_listallpids/proc_pidinfo since macOS has no /proc — scanProcessGroup/snapshotTree/groupRssBytes fold over them unchanged; `probeKqueue()` producer for the `kqueue` capability; substrate already rendered the macosx key set since C1a; `forcePollRequested()` lifted cross-platform; ladder flipped macosx→darwin) + 80f18dd (libproc buffer-sizing fix: incompleteStruct sizeof under-sized proc_pidinfo → read into a 512B buffer + cast) + 0b72a3f (macos timing honesty: B2 latency-discrimination gated `when defined(linux)` since macos-latest CI noise > the 25ms poll-tick delta; macos C1b suite reframed as functional prompt-detection). hlNetwork is the uniform never-goal (evidenceSatisfies refuses netIso on every platform — TEST-only assertion, no macOS prod code); setsid→toUnobservable falls out of the pgid-only domain (caps.subreaper=false) — TEST-only. Liveness: kqueue `next` exercised by every macos conformance spawn + the C1b timing suite; libproc forensics proven by test_conformance_forensics (real RSS+tree on Darwin AND Linux); `kqueue`==true asserted in the macos capabilities tier. CI run 34431503261 green ALL FIVE legs (test/cgroup/timing/windows/macos), 2026-09-09.

**Stage D — Windows backend (#15; conformance-green requires RFC-0009; leg + skeleton exist since A2d):**
- [x] **D1a** `process/windows.nim` core: `CreateProcess` + Job Object spawn, inherited handles to sinks, completion-port `next`, `reap` with exit codes + `ekNtStatus` symbol table, Job accounting rusage. The access-violation fixture lands HERE — it is `ekNtStatus`'s producer proof, not serde-only coverage. Conformance smoke green. ✅ commits 6b78a87 (ekNtStatus symbol table — `decodeExitCode` `>= 0xC0000000` → `ekNtStatus`) · 4e30a68 (`snapshotTree` forensics via `queryJobPids`/`snapshotOnePid`, tree=`toComplete`; completion-port `next` — `CreateIoCompletionPort` + `JobObjectAssociateCompletionPortInformation` + `GetQueuedCompletionStatus`, WFMO retired) · 1ba4dc2 (inherited sink handles proof + windows CI wiring, all 4 windows tests `if: always()`) · 702c40d (sweep gates exit-detection on `WaitForSingleObject(hProcess,0)==WAIT_OBJECT_0`, not exit code alone — fixes sink-handle teardown race + genuine-259 misread) · 34c6727 (access_violation fixture: non-nil unmapped address + `{.checks:off.}` + `-d:noSignalHandler` cfg → genuine uncaught `STATUS_ACCESS_VIOLATION`). The reap path is the uniform `sweepExitedChildren` (`GetExitCodeProcess` under the WFSO gate), never the IOCP message — the B1/C1b single-authoritative-sweep discipline on Windows. Liveness: windows leg drives all four through the real `Supervisor` — access_violation → `ekNtStatus 0xC0000005`, `oCrashed`; rss_hog → `snapshotTree` real pid/image/rssBytes; pass_fast → `weChildExited` via IOCP `next`; noisy_output → sink inherited byte-for-byte. crisol CLI unreachable at D1a (test suite still imports `std/posix`; RFC-0009). CI run 34437972869 green ALL FIVE legs (test/cgroup/timing/windows/macos), 2026-09-10.
- [x] **D1b** Kill domain + cooperative stop: `KILL_ON_JOB_CLOSE`, breakaway disabled, `CTRL_BREAK` gated on the probed deliverability (undeliverable ⇒ `evidence.cooperativeUnavailable`, straight to `TerminateJobObject(runner code)`) → forced kill; `Limits` → Job limits (`lkOpenFiles` = `lsUnsupported`; `LimitsAchieved` parent-computed, meaning documented); `JobObjectBasicProcessIdList` snapshots; Windows fixture variants (CTRL_BREAK handler, breakaway attempt, and the `DETACHED_PROCESS`/`FreeConsole` fixture — the only live producer of `cooperativeUnavailable: true`, asserted with `escalated` not degraded to noise); `capabilities()`/`substrate` extended. ✅ Delivered in three CI-verified sub-slices: **D1b-i** (c2b7c7c) — the real per-child `CTRL_BREAK` deliverability probe (`GetConsoleProcessList` contains the child's pid, replacing the D1a supervisor-global stub); `ctrl_break_handler` fixture (cooperative-success: escalated=false, cooperativeUnavailable=false, cbRunner, oKilled) + `detach_console` fixture (`FreeConsole` ⇒ the sole `cooperativeUnavailable:true` producer, escalated=false, forced kill, oKilled). **D1b-ii** (9c5ffad + flag-fix 50b55dc) — `lkCpu`→`PerProcessUserTimeLimit` + `lkAddressSpace`→`ProcessMemoryLimit` as real kernel-enforced Job limits (second non-fatal `SetInformationJobObject`), parent-computed `LimitsAchieved` (lsApplied=install-ok / lsFailed / lsUnsupported for lkFileSize/lkOpenFiles/lkCore/lkMemory); `cpu_spin` fixture proves enforcement (killed ~1s in, marker never written). **D1b-iii** (f208fbc) — `ppid` resolved via `CreateToolhelp32Snapshot` (replaces snapshotOnePid's -1); `breakaway_attempt` fixture proves the Job is a complete containment domain (an active `CREATE_BREAKAWAY_FROM_JOB` is denied → grandchild stays in the pid-list → `escapees == @[]` is honest, tree=toComplete). CI runs 34440588672 / 34444943333 / 34447537041 each green ALL FIVE legs (2026-09-10); also hardened a pre-existing C1b macOS forensics flake (b229dc2). **Known limitation (tracked, out of D1b's RFC scope):** cbLimit *attribution* on Windows is not modeled — `classifyCause` is signal-based (SIGXCPU→cbLimit), and Windows limit-kills carry no signal (`ekExited`/`ekNtStatus`), so a CPU-limit-killed test classifies `cbProcess` not `cbLimit(lkCpu)` even though `achieved[lkCpu]==lsApplied`. Closing the asymmetry needs decoding IOCP `JOB_OBJECT_MSG_END_OF_PROCESS_TIME` as an annotation + an additive `classifyCause limitKilled` param (mirroring `memoryOomKill`) — documented in process/windows.nim's header; awaits sign-off (touches the guarded "IOCP message is never decoded" discipline + a shared pure fn). **CLOSED by D1c (signed off and built 2026-09-18 — see the D1c bullet below).**
- [x] **D1c** cbLimit attribution on windows (the D1b follow-up, signed off 2026-09-18): `JOB_OBJECT_MSG_END_OF_PROCESS_TIME` decoded as an ANNOTATION (`ReapReport.limitKilled: Option[LimitKind]`, `some(lkCpu)`) — never a reap path (the sweep's process-handle readback still solely decides liveness; the guarded discipline is relaxed to annotation-only, deliberately). `classifyCause` gains additive `limitKilled: Option[LimitKind] = none` joined with requested+achieved exactly like B3's `memoryOomKill` (annotation without the join ⇒ `cbExternal`; a recorded stop act still wins — the ONE-owner rule unchanged). Race closed structurally: `reap` drains the completion port (timeout 0) BEFORE building the report, since the exit sweep can observe the process handle before the limit message is dequeued; per-pid guard so a same-Job grandchild exceeding its own per-process limit never annotates the direct child's reap. `lkAddressSpace` deliberately NOT attributed: `JOB_OBJECT_MSG_PROCESS_MEMORY_LIMIT` reports a failed allocation, not a kill — citing `cbLimit(lkAddressSpace)` from it would fabricate authorship. Unit table rows (5, mirroring B3's shape + exit-kind-agnosticism + stop-wins) in test_process_cause.nim; cpu_spin conformance case strengthened to assert `limitKilled == some(lkCpu)` and `cbLimit(lkCpu)` end-to-end; production call site (runner.toProcessResult) threads `report.limitKilled`. POSIX backends never set the field (ord-0 default `none`). ✅ 05a90cb, CI-proven on the windows leg, all 5 legs green (run 35353539907, 2026-09-18).
- [x] **D2a** Windows production de-POSIXing — the crisol `run` CLI now compiles AND runs on Windows end-to-end. ✅ **D2a-1** (3e9760f) process/windows.nim `globalShutdownSignal()` getter (the real "signals" work — signals.nim delegates, unchanged) + real `groupRssBytes` (live psapi sum over the Job pid-list). **D2a-2** (a519664) `lock` backend split — `lock/posix.nim` (verbatim) + `lock/windows.nim` (`LockFileEx`) + `lock.nim` ladder. **D2a-3** (07d6696) `memprobe.availableMemBytes` via `GlobalMemoryStatusEx`. **D2a-4** (f8c0ba4 + noFollow fix 08dbc89) `ioutils` de-POSIX via the Windows CRT int-fd backend (`CreateFileW`+`_open_osfhandle`, `_O_BINARY`, `GetFileAttributesW` pre-check for the `O_NOFOLLOW`/symlink-refusal analog) — the `cint` fd type is preserved so ledger/shardedledger are untouched. **D2a-5** (db6e723) `httpraw` socket timeouts (`SO_RCVTIMEO`/`SO_SNDTIMEO` as a winsock DWORD-ms optval, not a `timeval`) — the last std/posix import on the run path; also the windows `reap` briefly gained `runPhase` for backend signature parity (a documented no-op) — reverted by review r28 (2026-09-19), which moved the concept to `ChildSpec.claimOrphans` and restored the pinned `reap(sv, id)` signature. **D2a-6** (a7325e9) the LIVENESS proof: a windows conformance test builds the real CLI and runs `crisol run --json` on a throwaway project, asserting a real `summary.counts.passed==1` (`[OK] real crisol binary, real subprocess run, one entrypoint passes`), plus a dedicated windows CI step proving `src/crisol.nim` compiles for `--os:windows`. Each sub-slice green ALL FIVE legs (CI 34450315559/34455833979/34458389269/34464634346/34469194688/34471882898, 2026-09-10). **Scope note:** `httpraw` was an unnamed-in-this-bullet production blocker (posix socket timeouts) — folded into D2a. The A3 ownership allow-list gained `crisol/httpraw.nim` and its grep was hardened to catch bracketed `import std/[…posix…]` forms.
- [x] **D2b** Test-suite portability sweep + `windows-latest` conformance leg + windows green. **DONE under RFC-0009 (CI run 35170572732, all five legs green, 2026-09-16).** Landed via RFC-0009 Stage B: the test suite was de-POSIX'd (B1/B2/B3a/B3b/B3c buckets), the `windows-latest` leg was widened from the D2a CLI smoke to `tests/unit:tests/conformance` run natively through `ci/run-tests.sh`, and it takes full green under an HONEST scope. Precisely: the suite *compiles* for windows; the posix-backend categories (process/signal control, rlimit, and the POSIX process-backend conformance suite `test_conformance.nim`) *skip by design* on windows — that backend is covered by the per-file `test_windows_*` suite — enforced BY NAME through `ci/assert-subset-honesty.sh`; and path IDENTITY itself is proven by the natively-running `A3b-ii`/`A4b` conformance tests on windows NTFS (plus `A5c` on macOS APFS, since windows-latest lacks symlink-create privilege). This is NOT a claim that every test runs on windows — it is the honest completion the acceptance wording intended: identity green natively, posix-backend categories skipped by design.
