...
- [ ] **B2a** ✅ 35550d0, CI run 33882958224 green (all four legs) *(`cachetelemetry.nim` + `cachedispatch.nim` + `api.nim`/`runner.nim`/`cacheregistry.nim` threading)* `tekHit/tekMiss/tekRemoteErr/tekPublish/tekVerifyFail` emitted at the real consult (`lookupAtPlan`), store (`realSeams`), and verify (`verifyCachePass`) sites (`tekBackfillErr` lands with its producer in A3a); `CacheStats` aggregation (L1/remote/miss/remote-errors incl. trust codes/total/notConsulted/%/wall-saved/published/verifyFails) — via `InMemorySink`. **B2b** *(`api.nim` + `jsonout.nim` + `crisol.nim` + `config.nim`)* `RunReport.cacheStats` + run/v2 `cacheStats` (`schemaRevision` 21 — renumbered, see §Contract impacts) + `--cache-stats` + KDL `cache-stats` + summary line (stderr in `--json` mode) + the per-tier 100 %-error warning + `usage()`/header. E2E-B stats half.
