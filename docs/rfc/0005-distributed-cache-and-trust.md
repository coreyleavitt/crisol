...
**Rendering (round 3, fresh fact #2):** `kcNimVersion` prev/curr are multi-line `nim --version` text + `|` + binary hash; `kcCcVersion` is `<cc --version first line>|<ldd --version first line>` (ccprobe.nim — no binary hash; corrected from "likewise", which contradicted §Key portability and the code); `kcClosure`/`kcFlags` are opaque hashes. The renderer is component-aware: `kcNimVersion` = first line of the version text + 8-hex of the binary hash ("compiler binary differs" when only the hash segment changes); `kcCcVersion` = per-segment `cc: prev → curr` / `ldd: prev → curr` lines, no truncation, no "binary" language; `argv` joined; `limits` diffed per `LimitKind` (the enum-indexed `Limits.req` — rfc-0007 A2a-iii); `kcHermeticEnv` lists `envNames`; opaque hashes render as "changed (`a1b2…` → `c3d4…`)". Full values only under `--explain-miss-verbose`.
...
- [x] **B1** *(`keys.nim` + `cachelocalfs.nim` + `cachewire.nim` + `cachedispatch.nim` adapter + `crisol.nim` + `config.nim` + `types.nim` + render)* — split as **B1a** ✅ 99f7add, CI run 33839492789 green (all four legs) — pure `explainMiss` (+`envDigest` names) in `keys.nim`, exhaustively vector-tested (each of 9 components; flag-change; env-name; multi-component; no-diff); **B1b** ✅ 4df3cad (+ 16197ea localFsBackend v-dir fix), CI run 33845701136 green (all four legs) — the path-keyed sidecar under `inputs/` (tier 0 only; seeded on backfill; pruned in `gcResultCache`'s walk; mtime-ordered LRU) + `CacheLookup.explain` attached on miss — local-fs boundary suite; **B1c** ✅ 597b521, CI run 33859961081 green (all four legs) — `--explain-miss`/`--explain-miss-verbose` + KDL `explain-miss` + component-aware render (multi-line nimVersion rule; flag-gated diagnostic consult for non-edRunFresh entrypoints) + `usage()`/header. E2E-B explain half.
- [x] **B2a** ✅ 35550d0, CI run 33882958224 green (all four legs) *(`cachetelemetry.nim` + `cachedispatch.nim` + `api.nim`/`runner.nim`/`cacheregistry.nim` threading)* `tekHit/tekMiss/tekRemoteErr/tekPublish/tekVerifyFail` emitted at the real consult (`lookupAtPlan`), store (`realSeams`), and verify (`verifyCachePass`) sites (`tekBackfillErr` lands with its producer in A3a); `CacheStats` aggregation (L1/remote/miss/remote-errors incl. trust codes/total/notConsulted/%/wall-saved/published/verifyFails) — via `InMemorySink`. **B2b** ✅ 3fe07ef, CI run 33902160179 green (all four legs) *(`api.nim` + `jsonout.nim` + `crisol.nim` + `config.nim` + `render.nim` + `types.nim`)* `RunReport.cacheStats` + run/v2 `cacheStats` (`schemaRevision` 21 — renumbered, see §Contract impacts) + `--cache-stats` + KDL `cache-stats` + summary line (stderr in `--json` mode) + the per-tier 100 %-error warning + `usage()`/header. E2E-B stats half.
- [x] **A3a** ✅ 8ff9044, CI run 33916622279 green (all four legs) *(`cachetier.nim` + `cacheregistry.nim` + `cachetelemetry.nim` + `types.nim`)* multi-tier `lookup`: waterfall + backfill-on-hit + **`verified`-bit backfill rule** + **put rule** + per-tier `verdicts` + `worst` + **circuit breaker** + `drainPending` (budget-bounded deferred-put drain), via two `memory` tiers and the **controllable mock `TrustPolicy`**. Exhaustive 2×2×2 backfill matrix + 2×2 put matrix. `cacheregistry.nim`: `BackendRegistry` + scheme-resolved `buildBackend(RemoteTier, token)`; `productionRegistry()` (file) / `testRegistry()` (+memory/memoryBytes) — the `fetcher` param arrives with `HttpFetcher` in C1. *(As landed: the breaker is a permanent per-run latch per §TieredCache's normative "dead for the rest of the run" — the bullet's earlier "(fake clock)" was vestigial, no timed half-open exists to clock; `tekBackfillErr` + its pure producer `backfillErrEvents` over `CacheLookup.backfillVerdicts` landed here, live `sink.emit` wiring rides A3b's `realSeams` threading; the end-of-run deferred-put FLUSH wiring lands with the first real non-L1 tier in A3c-ii.)*
