...
##   3. Each entrypoint carries `flaky` boolean (B1 field, was missing from run/v1, carried into run/v2)
##   4. Each entrypoint carries `attempts` integer (B1 field, was missing from run/v1, carried into run/v2)
##   5. Summary carries `quarantined` integer count
##   6. RunSchemaRevision is >= 3 (was 3 after B3 bump; C5 bumped to 4)
##
## Run with:
