...
when defined(linux):
  ## RFC-0009 B4a: gated to Linux. availableMemBytes's `when defined(windows)`
  ## branch bypasses the injectable `read` seam entirely (documented in
  ## memprobe.nim: "read is unused here by construction... kept only for
  ## signature parity") and calls GlobalMemoryStatusEx directly -- so every
  ## test below that injects synthetic /proc+cgroup content via `read` is
  ## exercising Linux-only production logic and cannot pass (or mean
  ## anything) on Windows, where the seam is never consulted.
